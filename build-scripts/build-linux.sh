...
cp -f  shell/*.pak "$SERVER_FOLDER/bin/" || fail "Could not copy CEF PAK resources"
cp -f  shell/*.dat "$SERVER_FOLDER/bin/" || fail "Could not copy CEF DAT resources"
cp -f  shell/*.bin "$SERVER_FOLDER/bin/" || fail "Could not copy CEF BIN resources"
